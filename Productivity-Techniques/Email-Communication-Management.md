# Email & Communication Management

## Overview

Managing email and communication effectively to reduce overwhelm and improve response times.

## Email Management

### Inbox Zero
- Process to zero regularly
- Archive, delete, or act
- Use folders/labels strategically
- Unsubscribe from unnecessary emails

### Email Processing Workflow
1. **Delete**: Unnecessary emails
2. **Delegate**: Forward to appropriate person
3. **Respond**: Quick replies (< 2 minutes)
4. **Defer**: Schedule for later
5. **Do**: Take action if needed

### Email Templates
- Common responses
- Meeting requests
- Follow-ups
- Introductions

### Email Rules and Filters
- Automate sorting
- Priority inbox
- Unsubscribe management
- Spam handling

## Communication Protocols

- Response time expectations
- Communication channels (when to use what)
- Meeting vs. email vs. chat
- Status updates

## Boundary with Meetings

Use this page to decide whether email or another asynchronous channel is appropriate. For agendas, facilitation, action tracking, meeting-free time, and declining meetings, see [Meeting Effectiveness](./Meeting-Effectiveness.md).

## Resources

- [Inbox Zero — Merlin Mann](https://www.43folders.com/2006/09/06/inbox-zero-video)
- [Gmail filters and labels — Google Support](https://support.google.com/mail/answer/6576)
- [Email management — Harvard Business Review](https://hbr.org/2018/01/how-to-get-your-inbox-to-zero-every-day)
- [The 3-Email Rule — Business Insider](https://www.businessinsider.com/the-3-email-rule-2015-5)
